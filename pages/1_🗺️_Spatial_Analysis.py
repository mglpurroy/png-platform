...
with col1:
    start_year = st.selectbox(
        ...
        index=max(0, len(list(range(min_year, max_year + 1))) - 2) if max_year >= 2024 else 0,  # Default to 2024 if available
        key="start_year"
    )
...
with col2:
    ...
    current_end_month = st.session_state.get('end_month', min(len(end_month_options) - 1, 10) if max_end_month >= 11 else len(end_month_options) - 1)
